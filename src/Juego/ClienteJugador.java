...
//		envio
		...
		try {
			...
			barcosRival = ClienteJugador.tabla_rival();
			Tablero tableroRival;

			VentanaJugador.misBarcos.setText("<html><pre>" + Cliente.mostrarTablero(misBarcos) + "</pre></html>");
			VentanaJugador.barcosRival.setText("<html><pre>" + Cliente.mostrarTablero(barcosRival) + "</pre></html>");
			...
			while (barcosRival.isWin() == false) {
				VentanaJugador.btn_enviar.setEnabled(false);
				...
				if (i == 3) {
					...
					oos.writeObject(tableroRival);
					ClienteJugador.actualizarMisBarcos(misBarcos);
					// actualizo tablero rival
					while (ok == false) {
						VentanaJugador.txt_turno.setText("Tu turno");
						VentanaJugador.btn_enviar.setEnabled(true);
					}
					ClienteJugador.limpiar();
					// mando mi jugada
	...
	public static boolean comprobarMiTablero(Tablero barcosRival) {
		...
		if (barcosRival.getColumna() < 4 && barcosRival.getFila() < 4 && barcosRival.getColumna() >= 0
				&& barcosRival.getFila() >= 0) {
			switch (arrayBarcosRival[barcosRival.getFila()][barcosRival.getColumna()]) {
			case (char) tocado:
				VentanaJugador.txt_misAtaques.setText("Ya has seleccionado esta posición, prueba otra");
				return false;
			...
			VentanaJugador.txt_misAtaques.setText("No puedes atacar una posicion menor a 1 o mayor a 5");
			return false;
		}
	}
	...
	public static void comprobarTablero(Tablero misBarcos, Tablero tableroRival) {
		...
		// comprobamos la posicion que nos manda el rival en nuestro tablero
			...
			case barco:
				...
				i++;
				misBarcos.setLog("El rival ha atacado fila " + (tableroRival.getFila() + 1) + " columna "
						+ (tableroRival.getColumna() + 1) + " con el resultado: " + tocado);
				tableroRival.setLog("Has atacado file " + (tableroRival.getFila() + 1) + " columna "
						+ (tableroRival.getColumna() + 1) + " con el resultado: " + tocado);
				break;
			...
			case empty:
				...
				misBarcos.setLog("El rival ha atacado fila " + (tableroRival.getFila() + 1) + " columna "
						+ (tableroRival.getColumna() + 1) + " con el resultado: " + agua);
				tableroRival.setLog("Has atacado fila " + (tableroRival.getFila() + 1) + " columna "
						+ (tableroRival.getColumna() + 1) + " con el resultado: " + agua);
				break;
			}
		}
	...
	{
		VentanaJugador.misBarcos.setText("<html><pre>" + Cliente.mostrarTablero(misBarcos) + "</pre></html>");
		VentanaJugador.txt_ataquesRival.setText(misBarcos.getLog());
	}
	
	public static void actualizarBarcosRival(Tablero barcosRival)
	{
		VentanaJugador.barcosRival.setText("<html><pre>" + Cliente.mostrarTablero(barcosRival) + "</pre></html>");
		VentanaJugador.txt_misAtaques.setText(barcosRival.getLog());

	}
	
	private static void limpiar() {
		VentanaJugador.edt_columna.setText(null);
		VentanaJugador.edt_fila.setText(null);
	}
}
